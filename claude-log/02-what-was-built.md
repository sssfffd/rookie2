...
| 요구 | 어디에 |
|---|---|
| 레인 / 겹쳐보기 | `Controls/GraphCanvas.DrawLane()`, 위 도구 줄의 [보기 방식] |
...
| 요구 | 어디에 |
|---|---|
| 처음엔 아무것도 선택 안 됨 | `ChannelListVm` 의 선택 상태가 비어서 시작 |
| 전체 선택 / 전체 해제 | 왼쪽 위 버튼 |
| Shift 범위가 **보이는 순서**를 따름 | `ChannelListVm.SelectRange()` — `Rows` 의 줄 번호로 훑음 |
| 다른 IO 를 눌러도 이전 선택이 안 꺼짐 | `ChannelListVm.Toggle()` — 같은 것을 다시 눌러야 꺼짐 |
| 검색창이 깜빡이지 않음 | 검색창이 진짜 `TextBox` 라 그래프를 다시 그려도 영향 없음 (그래프는 별도 요소) |
| 타입 필터 | 왼쪽 위 콤보 (전체/디지털/아날로그/상태) |
| 고른 IO만 보기 | 왼쪽 위 콤보 (모든 IO / 고른 IO만 / 달라진 IO만) |
| 그리는 순서 = 목록 순서 | `ChannelListVm.DrawOrder()` 가 `Rows` 를 그대로 훑음 |

## 4. 그룹

| 요구 | 어디에 |
|---|---|
| 10개 단위 기본 그룹 | `ChannelListVm` 이 저장된 그룹이 없을 때만 10 개씩 묶음 |
...
| 버튼에 글자 | 기호만 있는 버튼을 안 만들었습니다 (`Themes/Controls.xaml` 머리말 참고) |

## 5. 두 로그 비교

| 요구 | 어디에 |
|---|---|
| 이전/이후 색 구분 | `Palette.Before` = 파랑, `Palette.After` = 주황 |
...
| 차이량 7가지 | 최대 / 평균 / RMS / 면적 / 시간비율 / 표본수 / 구간수 (`Compare/DiffTypes.cs`). 화면에는 면적을 빼고 6가지를 내놓습니다 ([09](09-axis-fit-and-toolbar.md)) |
...
| 요구 | 어디에 |
|---|---|
| 좁아지면 줄바꿈, 버튼이 안 튀어나옴 | XAML 의 `WrapPanel` |
| 버튼에 글자 | 전부 글자 |
| 진행 표시 + 취소 + 배경 스레드 | `Services/BackgroundJob.cs` + `Views/LoadingWindow.xaml` |
| 다크/라이트 테마 | `Themes/*.xaml`. 설정 창에서 전환하면 **바로** 바뀝니다 |
| 모니터별 DPI | `app.manifest` 의 PerMonitorV2 + `App.config` 의 DPI 스위치. WPF 는 단위가 DIP 라 배율 계산을 따로 하지 않습니다 |
...
| 요구 | 어디에 |
|---|---|
| 흰 바탕 + 진한 파랑 메뉴 | `Themes/Light.xaml` 의 `Brush.Window` = 흰색, `Brush.TopBar` = `#103562` |
| 대시보드가 먼저 | `ShellVm.Page` 가 대시보드로 시작 |
| 네모칸 요약 | `DashboardView.xaml` 의 네모 칸 4개 — 이전 IO 개수(이름이 이후에 없는 것 n개), 이후 IO 개수(이름이 이전에 없는 것 n개), 차이 발생 IO 개수, 표본 수 |
| 큰 네모칸에 차이 난 IO 목록 | `DashboardView.xaml` 의 `ChangedList` |
| IO 누르면 그래프로 | `DashboardView.IoActivated` → `GraphView.FocusOnIo(name)` |
| 메뉴로도 이동 | 위 메뉴 [대시보드] / [그래프] / [히트맵] (F1 / F2 / F3) |
| 그래프에서 직접 고르기 | 왼쪽 목록 (`GraphView.xaml` 의 `RowList` + 그룹 + 검색 + 필터) |

## 10. 히트맵 (나중에 추가된 요구)

| 요구 | 어디에 |
|---|---|
| 위 메뉴에서 갈 수 있는 새 화면 | 위 메뉴 [히트맵] (F3) |
| 차이가 나는 IO 를 왼쪽에 | `HeatmapCanvas.DrawNames()` — 많이 벌어진 IO 가 위로 |
| 가로축 = 시간 (1분 단위) | `HeatmapBuilder.ChooseBucketSpan()` — 시각 로그면 1분, 아니면 60칸 |
| 차이 발생 칸 = 빨간 네모 + 평균 차이값 | `HeatmapCanvas.OnRender()` — 진하기는 그 줄의 최대 대비 |
| 지표는 합계도 부호합도 아닌 것 | **절대 차이의 평균.** 이유는 [04-heatmap.md](04-heatmap.md) 1번 |
| 차이 없으면 "정상" 네모 | 같은 곳 |
| 0.1% 오차는 오류 아님 | `HeatmapBuilder.BuildThreshold()` — `max(절대 오차, 범위 x 0.1%)` |
