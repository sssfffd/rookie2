...
기본값은 **채널 값 크기의 0.1%** 입니다. 채널마다 값의 크기가 달라서
절대값 하나로는 다룰 수 없기 때문입니다.

### 무엇의 몇 % 인가

"몇 %" 를 곱할 밑값은 두 가지 중 **큰 쪽**입니다.
...
| 채널 | 폭 | 크기 | 밑값 | 1% 기준값 | 1 만큼 벌어지면 |
|---|---|---|---|---|---|
| 아날로그 0 ~ 4000 | 4000 | 4000 | 4000 | 40 | 잡음 — 안 셉니다 |
| 아날로그 6466 ~ 6467 | **1** | **6467** | 6467 | 64.7 | 잡음 — 안 셉니다 |
| 아날로그 0 ~ 100 | 100 | 100 | 100 | 1 | 경계 |
| 디지털 0 / 1 | 1 | 1 | 1 | 0.01 | 차이 — 완전히 다른 상태 |

둘째 줄이 중요합니다. 6466 과 6467 사이만 오가는 채널은 **폭이 1** 이라,
폭만 보면 1% 기준값이 0.01 이 되어 1 만큼의 차이가 차이로 잡혀 버립니다.
하지만 6467 짜리 값에서 1 이 흔들린 것을 1% 로 봐 달라는 말은 누가 봐도
"6467 의 1%" 라는 뜻입니다. 폭이 좁은 것은 **그 로그가 신호의 한 토막만
담고 있다**는 뜻이지, 그 채널이 원래 1 만큼만 움직인다는 뜻이 아닙니다.

좁은 폭에서 벌어진 작은 차이까지 봐야 한다면 비율을 낮추거나(예: 0.01%)
절대 허용 오차를 쓰면 됩니다. 상태(문자열) 채널에는 허용 오차를 매기지
않습니다 — 이름이 다르면 차이입니다.

### 실제로 쓰인 기준값을 보는 곳

"왜 이게 차이로 떴지" 가 궁금하면 그 채널에 **실제로 쓰인 기준값**을 보면
됩니다. 세 군데에 다 나옵니다.

- 대시보드 목록의 **기준값** 칸 (제목을 누르면 그 기준으로 정렬됩니다)
- 그래프 도구 줄의 안내 글
- 히트맵에서 칸에 마우스를 올렸을 때의 아래 띠
...
자세한 이유는 [claude-log/05-tolerance.md](claude-log/05-tolerance.md) 와
[claude-log/12-tolerance-base.md](claude-log/12-tolerance-base.md).
...
위에서부터 **크게 벌어진 IO** 순입니다. 기준은 "가장 크게 벌어진 순간 ÷ 그
채널의 값 범위" 이고, 같으면 기준을 넘은 표본이 많은 쪽, 그 다음은 이름순입니다.
